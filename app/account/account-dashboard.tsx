  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (status === "authenticated") {
        // Remove admin check and redirect
        try {
                  ...
                  <div className="flex flex-col items-center space-y-4">
                    <div className="relative">
                      <Avatar className="h-24 w-24">
                        <AvatarImage src={user?.image || "/mohitsarees/placeholder-simple.svg"} alt={user?.name} />
                    ...
                    {user.wishlist.slice(0, 2).map((item: any) => {
                      ...
                      <div key={item.id} className="flex gap-4">
                        <div className="relative w-16 h-16 rounded overflow-hidden flex-shrink-0">
                          <Image
                            src={item.product?.imageUrls ? JSON.parse(item.product.imageUrls)[0] : "/mohitsarees/placeholder-simple.svg"}
